...
2. **Saves to Organization Memory**
   - Appends to `.claude/memory/org/decisions.json`
...
✅ Decision saved to .claude/memory/org/decisions.json
...
Saved as JSON in `.claude/memory/org/decisions.json`:
...
Decisions are:
- ✅ Committed to git (`.claude/memory/org/decisions.json`)
...
const orgMemory = JSON.parse(
  await Read('.claude/memory/org/decisions.json')
);
...
// Save back
await Edit('.claude/memory/org/decisions.json', 
