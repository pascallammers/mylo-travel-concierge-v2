...
Loads team-wide knowledge from `.claude/memory/org/`:
...
Loads personal preferences from `.claude/memory/user/{username}.json`:
...
const userMemory = JSON.parse(
  await Read(`.claude/memory/user/${username}.json`)
);
