...
#   orchestrator.sh --spec .claude/specs/active/feature-auth.md
...
start_claude_in_session() {
    ...
    step "Starting Claude Code with $specialist in $session_name"

    # Send Claude Code command to tmux session
    # Note: This requires Claude Code CLI to be installed and in PATH
    tmux send-keys -t "$session_name:0" "# Starting Claude Code with $specialist specialist..." C-m
    tmux send-keys -t "$session_name:0" "# Read .claude-context.md for your task instructions" C-m
    tmux send-keys -t "$session_name:0" "cat .claude-context.md" C-m

    # Optionally start Claude Code automatically
    # Uncomment the following line if you want Claude to start automatically
    # tmux send-keys -t "$session_name:0" "claude --agent $specialist" C-m

    success "Claude Code ready in session: $session_name"
    ...
    for i in $(seq 0 $((num_tasks - 1))); do
        ...
        if worktree_path=$(create_worktree "$task_key" "$task_title" "$specialist"); then
            ...
            if session_name=$(create_tmux_session "$task_key" "$worktree_path" "$specialist" "$task_desc"); then
                ...
                # Start Claude in session (optional - can be manual)
                # start_claude_in_session "$session_name" "$specialist" "$worktree_path"
            fi
        fi
    done
...
${BOLD}EXAMPLES:${NC}
  ...
  # From spec file
  $0 --spec .claude/specs/active/feature-auth.md
