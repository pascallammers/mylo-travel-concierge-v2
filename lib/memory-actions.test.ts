  ...
  namedExports: {
    Supermemory: class {
      memories = {
        ...
        list: async (input: { containerTags: string[] }) => {
          assert.deepEqual(input.containerTags, ['owner']);
          return { memories: [], pagination: { totalItems: 0 } };
        },
        add: async (input: { containerTag: string }) => {
          assert.equal(input.containerTag, 'owner');
          return { id: 'memory' };
        },
      };
      search = {
        memories: async (input: { containerTag: string }) => {
          assert.equal(input.containerTag, 'owner');
          return { total: 0 };
        },
      };
...
for (const name of ['searchMemories', 'getAllMemories'] as const) {
  test(name + ': anonymous is rejected', async () => {
    ...
    await assert.rejects(
      () => Reflect.apply(actions[name], undefined, name === 'searchMemories' ? ['travel'] : []),
      /Authentication required/,
    );
  });
  test(name + ': uses the session container', async () => {
    assert.deepEqual(await Reflect.apply(actions[name], undefined, name === 'searchMemories' ? ['travel'] : []), {
      memories: [],
      total: 0,
    });
  });
