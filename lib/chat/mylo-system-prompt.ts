...
function buildKbFirstAndRouting(): string {
  ...
  - ⚠️ DOMAIN TOOLS WIN — skip \`knowledge_base\` and call the right domain tool directly when the query has clear domain intent:
    ...
    * **Hotel search** ("Hotel in/nahe X") → \`trivago_hotel_search\` (if available); pass only the place as query (e.g. "Brandenburger Tor", never "Hotel nahe Brandenburger Tor"); results are already in EUR and German, show each hotel as its own card with its photo and trivago link and quote prices exactly as returned; no hotels found is not an outage: suggest a different or larger place
