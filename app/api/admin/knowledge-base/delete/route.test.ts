...
function request() {
  return new NextRequest('http://localhost/api/admin/knowledge-base/delete', {
    method: 'POST',
...
for (const state of ['no session', 'non-admin', 'admin'] as const) {
  test('DELETE legacy: ' + state, async () => {
    ...
    const response = await route.DELETE(
      new NextRequest('http://localhost/api/admin/knowledge-base/delete?name=files/document', { method: 'DELETE' }),
    );
    assert.equal(response.status, state === 'no session' ? 401 : state === 'non-admin' ? 403 : 200);
