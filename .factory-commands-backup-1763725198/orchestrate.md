...
# From spec file
/orchestrate spec:.factory/specs/active/feature-auth.md
...
- Task instructions in `.factory-context.md`
...
<execute>
# Find project root (works with both Claude Code and Factory.ai Droid CLI)
PROJECT_ROOT="${DROID_PROJECT_DIR:-${CLAUDE_PROJECT_DIR:-$(pwd)}}"
ORCHESTRATOR_SCRIPT="$PROJECT_ROOT/.factory/scripts/orchestrator.sh"

if [ ! -f "$ORCHESTRATOR_SCRIPT" ]; then
  echo "❌ Orchestrator script not found: $ORCHESTRATOR_SCRIPT"
  echo ""
  echo "Please ensure the orchestrator script exists."
  echo "Expected at: $PROJECT_ROOT/.factory/scripts/orchestrator.sh"
  exit 1
fi
...
if [ -z "$ARGUMENTS" ]; then
  ...
  echo "  2. Spec file (e.g., '.factory/specs/active/auth.md')"
